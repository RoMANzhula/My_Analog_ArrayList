...
public class AnalogArrayList<T> {
    ...
    public void print() {//метод для вывода на консоль массива
        ...
        while (iterator.hasNext()) {//проверяем есть ли еще элементы в массиве
            ...
            if (object != null) System.out.println(object);//проверяем чтобы элемент не был null - печатаем данный элемент
        }
    }
    ...
    public boolean contains(T elementOfArray) {//метод для сравнения обьектов в массиве
        int indexOf = 0;//создаем переменную (псевдо-метод indexOf) и инициализируем ее 0
        for (Object element : arrayObjects) {//циклом for-each (для каждого єлемента из массива) проходим по массиву
            if (element.equals(elementOfArray)) {//если єлемент массива будет равен сравниваемому єлементу, то
                indexOf++;//инкрементируем переменную (псевдо-метод), главное чтоб она не имела отрицательное значение(т.е. true)
            } else {//иначе
                indexOf = -1;//присваиваем переменной (псевдо-методу) значение -1 (т.е. false)
            }
        }
        return indexOf >= 0;
    }
